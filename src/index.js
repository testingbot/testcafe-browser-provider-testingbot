...
module.exports = {
  ...
  async openBrowser(id, pageUrl, browserName) {
    if (!browserName) throw new Error("Browser not specified!");

    if (!process.env.TB_KEY || !process.env.TB_SECRET)
      throw new Error(
        "Please specify both TB_KEY and TB_SECRET environment variables. You can find these two keys in the TestingBot member area.",
      );

    if (!process.env["TB_SKIP_TUNNEL"]) this.tunnel = await this.startTunnel();
    ...
    if (this.heartbeatInterval > 0) this.startHeartbeat(id, webDriver);
  },

  async startTunnel() {
    this.tunnelIdentifier =
      process.env.TB_TUNNEL_IDENTIFIER || "testcafe-" + new Date().getTime();
  ...
  async closeBrowser(id) {
    if (this.heartbeatInterval > 0) this.stopHeartbeat(id);

    if (this.openedBrowsers[id]) await this.openedBrowsers[id].quit();
  ...
  generateCapabilities(browserName, id) {
    ...
    if (manualCaps) {
      if (platform && platform.length > 0) caps["platformName"] = platform;

      if (version && version.length > 0) caps["browserVersion"] = version;

      if (browser && browser.length > 0) caps["browserName"] = browser;
    }
    ...
    if (process.env["TB_SCREEN_RESOLUTION"])
      caps["tb:options"]["screen-resolution"] =
        process.env["TB_SCREEN_RESOLUTION"];
  ...
  async dispose() {
    ...
    if (this.tunnel) await this.stopTunnel();
  },
    ...
    return new Promise((resolve, reject) => {
      this.apiClient.getBrowsers(function (error, browsers) {
        ...
        const browserList = browsers
          .filter((browser) => !browser.deviceName)
          .map((browser) => {
  ...
  async resizeWindow(id, width, height, currentWidth, currentHeight) {
    const currentWindowSize = await this.openedBrowsers[id]
      .manage()
      .window()
      .getRect();
    const currentClientAreaSize = {
      width: currentWidth,
      height: currentHeight,
    };
    const requestedSize = { width, height };
    const correctedSize = this.getCorrectedSize(
      currentClientAreaSize,
      currentWindowSize,
      requestedSize,
    );

    await this.openedBrowsers[id]
      .manage()
      .window()
      .setRect({ width: correctedSize.width, height: correctedSize.height });
  },
  ...
  async takeScreenshot(id, screenshotPath /* , pageWidth, pageHeight */) {
    const screenshot =
      await this.openedBrowsers[id].takeScreenshot(screenshotPath);
  ...
  async reportJobResult(id, jobResult, jobData) {
    if (
      jobResult !== this.JOB_RESULT.done &&
      jobResult !== this.JOB_RESULT.errored
    )
      return null;
    ...
    if (this.openedBrowsers[id]) {
      ...
      const sessionId = session.getId();
      const jobPassed =
        jobResult === this.JOB_RESULT.done && jobData.total === jobData.passed;
  ...
  async _updateJobStatus(sessionId, passed) {
    return new Promise((resolve, reject) => {
      ...
      this.apiClient.updateTest(
        testData,
        sessionId,
        function (error, testDetails) {
          if (error) reject(error);
          else resolve(testDetails);
        },
      );
    });
  },
